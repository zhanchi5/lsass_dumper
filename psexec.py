        ...
        try:
            ...
            if self.__exeFile is None:
                installService = serviceinstall.ServiceInstall(
                    rpctransport.get_smb_connection(),
                    remcomsvc.RemComSvc(),
                    self.__serviceName,
                )
            else:
            ...
            if self.__copyFile is not None:
                ...
                self.__command = (
                    os.path.basename(self.__copyFile) + " " + self.__command
                )
            ...
            if self.__copyFile is not None:
                ...
                s.deleteFile(
                    installService.getShare(), os.path.basename(self.__copyFile)
                )
            unInstalled = True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        add_help=True, description="PSEXEC like functionality example using RemComSvc."
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=" ",
        help="command (or arguments if -c is used) to execute at "
    ...
    parser.add_argument(
        ...
        action="store",
        metavar="pathname",
        help="copy the filename for later execution, "
